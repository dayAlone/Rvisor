...
$.initializeLogin = function() {
	...
	$.notification( 
		{
			title: "Добро пожаловать в Rvision",
			content: "Самое время авторизоваться!",
	...
	$("#password button").bind("click tap", function() {
		forgot($(this));
        return false;
	});
	...
	function forgot(x) {
        if($("#password .input.username input").attr("value")=='') {
            $.notification(
                {
                    title: "Не правильное имя пользователя",
                    content: "Вы не заполнили поле.",
                    icon: "",
                    timeout: 2000
                }
            );
            drag();
            $("#password .input.username input").attr("value", "").focus();
        }
		else if($("#password .input.password input").attr("value")=='') {
			$.notification( 
				{
					title: "Не правильный пароль",
					content: "Вы не заполнили поле с паролем.",
					icon: "",
                    timeout: 2000
				}
			);
            ...
            //$(x).parents('form').submit();
            $.ajax({
                url: $(x).parents('form').attr('action'),
                type: 'post',
                data: {_username:$("#password .input.username input").val(),_password:$("#password .input.password input").val()},
                success: function(data){
                    if(!data.success) {
                        console.log(data);
                        $.notification(
                            {
                                title: "Не правильный логин или пароль",
